                        ...
                        <div class="address-option">
                            <div class="address-title">
                                <h4>Billing Address</h4><a href="#" data-bs-toggle="modal"
                                    data-bs-target="#create-address-modal" title="Add new address" tabindex="0">+ Add New
                                    Address</a>
                            </div>
                            ...
                            <div class="row gy-3">
                                <div class="col-sm-6">
                                    <label class="payment-box" for="cod">
                                        <input class="custom-radio me-2" id="cod" type="radio" name="radioPayment">
                        ...
                        <div class="cart-listing">
                            <ul>
                                ...
                                <li class="position-relative variant_item justify-content-between">
                                    ...
                                    <input type="hidden" class="variant_values" value="{{$item['variant_values']}}">
                                    <input type="hidden" class="variant_quantity" value="{{$item['quantity']}}">
                                    <input type="hidden" class="variant_price" value="{{$item['price']}}">
                                    <input type="hidden" class="variant_total_price" value="{{$item['quantity']*$item['price']}}">
                                    <div class="d-flex flex-row align-items-center">
                                        <img src="{{asset('uploads/products/images/'.$item['image'])}}" width="60px" height="64px" alt="">
                                        <div class="ms-2">
                                    ...
                                    <div class="d-flex flex-column align-items-end">
                                        <p class="text-nowrap" style="font-size: 14px;">{{number_format($item['price'],0,'.',',')}}đ</p>
                                        <p class="text-nowrap p_value_reduced hidden" style="font-size: 13px;">-<span class="value_reduced text-success">{{number_format($item['price'],0,'.',',')}}đ</span></p>
                                    </div>
                                    <span class="position-absolute top-0 end-0 rounded-start ps-1 pe-1 text-white" style="background-color:#cca270">x{{$item['quantity']}}</span>
                                </li>
                                @endforeach
                                @endif
                            </ul>
                            <div class="summary-total">
                                <ul>
                                    ...
                                    <li class="li_sub_total">
                                        <p>Subtotal</p>
                                        <div class="d-flex flex-column align-items-end">
                                            <span id="sub_total">{{number_format($check_out_data['sub_total'],0,'.',',')}}đ</span>
                                            <span class="hidden text-danger fw-bold" id="new_sub_total">{{number_format($check_out_data['sub_total'],0,'.',',')}}đ</span>
                                        </div>
                                        <input id="base_sub_total" type="hidden" value="{{$check_out_data['sub_total']}}">
                                    </li>
                                    <li>
                                        <p>Shipping</p>
                                        <span>{{number_format($check_out_data['shipping_fee'],0,'.',',')}}đ</span>
                                        <input type="hidden" id="shipping_price" value="{{$check_out_data['shipping_fee']}}">
                                    </li>
                                    <li>
                                        <p>Tax <span class="color-of-theme" style="font-size:10px;">(0,5% tổng giá trị đơn hàng)</span></p>
                                        <span>{{number_format($check_out_data['tax'],0,'.',',')}}đ</span>
                                        <input id="base_tax" type="hidden" value="{{$check_out_data['tax']}}">
                                    </li>

                                    <span class="mt-2 mb-2 d-block {{$check_out_data['free_ship']!="true"?"hidden":""}}" id="titleAppliedDiscounts">Applied Discounts</span>

                                    <li id="liShippingVoucher" class="{{$check_out_data['free_ship']!="true"?"hidden":""}}">
                                        <p>Shipping voucher</p>
                                        <input type="hidden" id="free_ship_status" value="{{$check_out_data['free_ship']}}">
                                        <span class="color-of-theme">
                                            -
                                            <span class="color-of-theme" id="shipping_voucher">{{number_format($check_out_data['minimum_payment_for_free_ship']>$check_out_data['sub_total']?0:$check_out_data['shipping_fee'],0,'.',',')}}đ</span>
                                        </span>
                                    </li>
                                    <li id="liVoucher" class="hidden">
                                        ...
                                        <span class="color-of-theme">
                                            -
                                            <span class="color-of-theme" id="voucher">{{number_format($check_out_data['shipping_fee'],0,'.',',')}}đ</span>
                                        </span>
                                    </li>
                                ...
                                <div class="coupon-code">
                                    <div class="position-relative divInputVoucher">
                                        ...
                                        <div class="position-absolute w-100 h-100 bg-dark-subtle top-0 d-flex flex-row justify-content-between align-items-center hidden" id="showVoucher">
                                            <label class="ms-3 m-0" id="voucherCode">giam100k</label>
                                            <span class="me-3">
                                                <i class="fas fa-xmark cspt fa-2xl color-of-theme" id="cancelVoucher"></i>
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <p style="font-size: 12px;" class="text-danger">
                                        ...
                                        Mua thêm {{number_format($check_out_data['minimum_payment_for_free_ship']-$check_out_data['sub_total'],'0','.',',')}}đ để nhận ngay <b>Ưu đãi miễn phí vận chuyển.</b>!
                                    </p>
                                </div>
                                @else
                                <div>
                                    <p style="font-size: 12px;" class="text-success">
                                        <i class="fa-solid fa-circle-exclamation color-of-theme"></i>
                                        Bạn đã nhận được <b>Ưu đãi miễn phí vận chuyển</b> với đơn hàng trị giá trên {{number_format($check_out_data['minimum_payment_for_free_ship'],'0','.',',')}}đ!
                                    </p>
                                </div>
                                @endif
                                @else
                            ...
                            <div class="total">
                                ...
                                <div class="total m-0 mb-3 d-flex flex-column align-items-end">
                                    <h6 class="" id="paymentTotal">{{number_format($check_out_data['total'],0,'.','.')}}đ</h6>
                                    <h6 class="text-danger fw-bold hidden" id="newPaymentTotal">{{number_format($check_out_data['total'],0,'.','.')}}đ</h6>
                                </div>
                            </div>
                            <div class="order-button">
                                <a class="btn btn_black sm w-100 rounded" href="javascript:void(0)" id="place_order">Place Order</a>
                            </div>
                        </div>
                    </div>
                </div>
