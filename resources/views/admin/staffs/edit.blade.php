    ...
    <div class="card-body">
        <form action="{{ route('admin.staffs.update', $staff->id) }}" method="POST">
            ...
            <div class="mb-3 mt-3">
                ...
                <input type="text" value="{{ old('full_name', $staff->full_name) }}" name="full_name" id="full_name" class="form-control @error('full_name') is-invalid @enderror" placeholder="Họ và tên...">
                @error('full_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
